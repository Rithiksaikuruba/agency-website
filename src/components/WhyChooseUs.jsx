...
import {
  motion, useMotionTemplate, useMotionValue, animate,
  useInView, AnimatePresence, useReducedMotion
} from 'framer-motion';
import {
...
const GlobalStyles = memo(() => (
  <style>{`
    ...
    *, body { font-family: var(--font); }
    ...
    /* ── Ticker — GPU-composited on its own layer ── */
    @keyframes ticker {
    ...
    /* ── Float badge — GPU layer ── */
    @keyframes float {
      0%, 100% { transform: translate3d(0, 0px,   0) rotate(-3deg); }
    ...
    /* ── Glow card — contain isolates repaint ── */
    .glow-card {
    ...
    /* ── Progress ring — only stroke-dashoffset animates ── */
    .progress-ring {
      transform: rotate(-90deg);
      transform-origin: 50% 50%;
      will-change: auto;
    }
    .progress-ring circle { will-change: stroke-dashoffset; }

    /* ── Blob backgrounds — composited on own layer ── */
    .bg-blob {
      will-change: transform;
      backface-visibility: hidden;
...
const AnimatedText = memo(({ text, className, delay = 0 }) => {
  ...
  const container = useMemo(() => ({
    hidden:  { opacity: 0 },
    visible: { opacity: 1, transition: { staggerChildren: shouldReduce ? 0 : 0.09, delayChildren: delay } },
  }), [delay, shouldReduce]);

  const child = useMemo(() => ({
    visible: { opacity: 1, y: 0,  transition: { type: 'spring', damping: 14, stiffness: 120 } },
    hidden:  { opacity: 0, y: shouldReduce ? 0 : 24, transition: { type: 'spring', damping: 14, stiffness: 120 } },
  }), [shouldReduce]);

  return (
    <motion.div
      style={{ overflow: 'hidden', display: 'flex', flexWrap: 'wrap', willChange: 'opacity' }}
...
const SpotlightCard = memo(({ children, className = '' }) => {
  ...
  // Throttle to rAF to avoid thrashing on fast mouse moves
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: gradientLight, willChange: 'background' }}
      />
      <motion.div
        ...
        style={{ background: gradientDark, willChange: 'background' }}
      />
...
const RingProgress = memo(({ value, size = 56, stroke = 4, color = '#6366f1', bg = 'rgba(99,102,241,0.1)' }) => {
  ...
  return (
    <div ref={ref} style={{ width: size, height: size, willChange: 'auto' }}>
...
const TickerStripe = memo(() => (
  <div className="w-full overflow-hidden border-y border-slate-100 dark:border-white/[0.05] py-3.5 mb-20 relative">
    ...
    {/* ticker-track runs a pure CSS @keyframes on transform — compositor thread only */}
...
/* ─── 9. ANIMATED CLOCK — RAF-based, no setState thrash ─────────── */
const AnimatedClock = memo(() => {
  // Store tick count in a ref; only setState when the SVG lines need updating
  ...
  const hour = (tick / 720) * 360;
  const min  = ((tick % 60) / 60) * 360;

  return (
    <div className="relative w-24 h-24 mx-auto" style={{ willChange: 'auto' }}>
    ...
    {bars.map((h, i) => (
      <div key={i} className="flex-1 rounded-t relative" style={{ height: '100%', background: 'rgba(99,102,241,0.06)' }}>
        <motion.div
          ...
          style={{
            background: `linear-gradient(to top, ${barColors[i % 4]}, ${barColors[(i+1) % 4]}80)`,
            willChange: 'height',
          }}
          initial={{ height: 0 }}
...
/* ── Animated chat widget — fully isolated, zero parent re-renders ── */
const AnimatedChatWidget = memo(() => {
  ...
  return (
    <div className="widget-isolate bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-white/[0.09] rounded-2xl shadow-xl shadow-slate-200/60 dark:shadow-black/40 overflow-hidden w-full">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-white/[0.06]">
        ...
        >
          ...
          <motion.div
            className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-500 rounded-full flex items-center justify-center"
      ...
      <div className="px-3 py-2.5 border-t border-slate-100 dark:border-white/[0.06] flex items-center gap-2">
        <div className="flex-1 h-8 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center px-3 gap-2 overflow-hidden">
          ...
          />
          <motion.span
            className="text-slate-300 dark:text-slate-600 text-sm leading-none"
  ...
  return (
    <div ref={ref} className="flex items-center gap-2">
      <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} fill="none">
        ...
        <motion.circle
          cx={w} cy={h - ((SPARK[SPARK.length-1] - min) / (max - min)) * h}
          r="2.5" fill="#22c55e"
...
/* ── Live activity feed — isolated, own contain context ── */
const LiveFeed = memo(() => {
...
const PostLaunchCard = memo(() => (
  <SpotlightCard className="md:col-span-2 lg:col-span-3 rounded-[2rem] p-6 sm:p-8 relative overflow-hidden">
    ...
    {/* Blob — GPU layer via will-change + translate3d */}
    <motion.div
      className="bg-blob absolute -right-32 -top-32 w-[500px] h-[500px] rounded-full blur-[100px] pointer-events-none opacity-60"
      style={{
    ...
    <div className="relative z-10 flex flex-col lg:flex-row gap-8">
      ...
      <motion.div
        className="flex-shrink-0 w-full lg:w-[268px]"
...
/* ─── 15. TYPED CODE BLOCK — isolated, 75ms interval stays local ── */
...
const TypedCodeBlock = memo(() => {
  ...
  useEffect(() => {
    // Use setInterval but keep mutable state in refs to avoid closure stale-value issues
    timerRef.current = setInterval(() => {
      ...
      setTypedCode(CODE_STRING.slice(0, iRef.current));
      iRef.current++;
      if (iRef.current > CODE_STRING.length) {
        setTimeout(() => { resetRef.current = true; }, 2800);
      }
    }, 75);
...
export default function WhyChooseUs() {
  ...
  // Background blob animations — disabled for prefers-reduced-motion
  const blobAnim1 = shouldReduce ? {} : { x: [0, 30, 0], y: [0, 20, 0] };
  const blobAnim2 = shouldReduce ? {} : { x: [0, -20, 0], y: [0, -30, 0] };

  return (
    <section
      className="relative py-24 sm:py-32 px-4 sm:px-6 overflow-hidden bg-slate-50 dark:bg-[#020617]"
      id="why"
      /* removed transition-colors duration-500 — triggers style recalc on theme change */
    >
      <GlobalStyles />

      {/* ── Background layers — blobs on their own GPU layers ── */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        {/* Blob 1 — translate only (compositor-thread) */}
        <motion.div
          className="bg-blob absolute top-[8%] left-[-15%] w-[750px] h-[750px] rounded-full blur-[140px]"
        ...
        {/* Blob 2 */}
        <motion.div
          className="bg-blob absolute bottom-[5%] right-[-10%] w-[650px] h-[650px] rounded-full blur-[130px]"
          style={{ background: 'radial-gradient(circle, rgba(168,85,247,0.08), transparent 70%)' }}
          animate={blobAnim2}
          transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
        />
        {/* Blob 3 — static, no animation needed */}
                ...
                <div className="w-full bg-slate-50 dark:bg-[#020617] pt-7 p-3 space-y-2.5">
                  <div className="flex justify-between items-center px-1 mb-1">
                    ...
                    <div className="flex gap-1">
                      {[...Array(4)].map((_, i) => (
                        <motion.div key={i}
                          className="w-1 rounded-sm bg-indigo-400"
                          style={{ height: `${(i + 1) * 2.5}px`, willChange: 'opacity' }}
          ...
          <SpotlightCard className="md:col-span-2 lg:col-span-2 rounded-[2rem] p-6 sm:p-8 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="space-y-0.5">
                <ComparisonRow label="Fixed-price contracts"       us={true}  them={false} delay={0.10} />
                <ComparisonRow label="6-month post-launch support" us={true}  them={false} delay={0.15} />
                <ComparisonRow label="Mobile-first approach"       us={true}  them={true}  delay={0.20} />
                <ComparisonRow label="Dedicated project manager"   us={true}  them={false} delay={0.25} />
                <ComparisonRow label="WCAG 2.2 compliance"         us={true}  them={false} delay={0.30} />
                <ComparisonRow label="Weekly progress updates"     us={true}  them={false} delay={0.35} />
              </div>
            </div>
          </SpotlightCard>

          {/* ── CARD 5: WORLD-CLASS STANDARDS ── */}
